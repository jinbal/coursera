...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender {
  ...
  test("insert the same should return OperationFinished") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
    topNode ! Insert(testActor, id = 1, 0)
    expectMsg(OperationFinished(1))
  }

  test("insert larger than root should return OperationFinished") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
    topNode ! Insert(testActor, id = 1, 1)
    expectMsg(OperationFinished(1))
  }

  test("insert less than than root should return OperationFinished") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
    topNode ! Insert(testActor, id = 1, -1)
    expectMsg(OperationFinished(1))
  }

  test("remove should remove") {
    val topNode = system.actorOf(Props[BinaryTreeSet])
    topNode ! Insert(testActor, id = 1, 1)
    expectMsg(OperationFinished(1))
